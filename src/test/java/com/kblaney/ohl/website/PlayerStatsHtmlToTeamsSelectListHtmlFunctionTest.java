...
public final class PlayerStatsHtmlToTeamsSelectListHtmlFunctionTest
{
  ...
  {
    final String playerStatsHtml = "Before<SELECT name=\"subStat\">Hello</SELECT>After";
    assertEquals("<SELECT name=\"subStat\">Hello</SELECT>", function.apply(playerStatsHtml));
  }

  @Test
  public void apply_teamSelectListContainsNewline()
  {
    final String playerStatsHtml = "Before<SELECT name=\"subStat\">A\nB\nC</SELECT>After";
    assertEquals("<SELECT name=\"subStat\">A\nB\nC</SELECT>", function.apply(playerStatsHtml));
  }

  @Test
  public void apply_twoSelectEndTags()
  {
    final String playerStatsHtml = "Before<SELECT name=\"subStat\">A</SELECT>After<SELECT>B</SELECT>";
    assertEquals("<SELECT name=\"subStat\">A</SELECT>", function.apply(playerStatsHtml));
  }
}
